...
@app.route('/users/<email>/gallery', methods=['GET'])
def get_user_gallery(email):
...
@app.route('/users/<email>/fits', methods=['POST'])
def save_fit(email):
    print("Saving fit...")
    data = request.json
    if not data:
...
@app.route('/users/<email>/fits/<int:index>', methods=['DELETE'])
def delete_fit(email, index):

    user = users.find_one({'username' : email})
    
    if not user or 'fits' not in user or index >= len(user['fits']):
        return jsonify({'status': 'error', 'message': 'Fit not found'}), 404
    
    new_fits = user['fits'][:index] + user['fits'][index + 1:]
    users.update_one({'username': email}, {"$set": {"fits": new_fits}})
    return jsonify({'status': 'success', 'message': 'Fit deleted successfully'}), 200
...
@app.route("/images/find-match", methods=["POST"])
def find_match():
    print("Finding match...")
    if not SERPAPI_KEY:
