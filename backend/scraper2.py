...
# Create folders
os.makedirs("images/tops", exist_ok=True)
os.makedirs("images/bottoms", exist_ok=True)
...
def scrape_images(url, save_folder):
    try:
        with sync_playwright() as p:
            ...
            idx = 0
            for url in img_urls:
                 open(f"images/{save_folder}/{save_folder}_{idx}.jpg", "wb").write(requests.get(url).content)
                 idx += 1
        return True
...
def setup_links(gender, formality):
    gender = gender.lower()
